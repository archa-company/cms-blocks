...
export default async function Sponsors({
  ...
  return (
    <div className={classNames('relative my-4', classData)}>
      {type == 'all' && (
        <>
          <p className="relative -bottom-4 mx-4 text-[27px] font-medium ">
            Parceiros
          </p>
          <div
            ...
            {taxo?.meta?.sponsorships &&
              taxo?.meta?.sponsorships?.map((x: any, y: number) => {
                return (
                  <div
                    className="flex w-[109px] flex-col items-center justify-around"
                    key={y}>
                    <a
                      href={x?.url_sponsorship}
                      target="_blank"
          ...
          return (
            <div
              ...
              <a
                ...
                <div className="relative h-full min-h-20 w-full  bg-white object-cover">
                  <img
                    src={x?.image_sponsorship}
                    alt={x?.name_sponsorship}
                    className="h-auto w-full"
...
async function getAllSponsors(categorie: string) {
  let sponsors = await fetch(
    `https://${process.env.HERMES_S3_BUCKET}.s3.amazonaws.com/${
      process.env.HERMES_S3_PREFIX
    }/json/routes${categorie.startsWith('/category') ? '' : '/category'}${
      categorie.endsWith('/')
        ? categorie.slice(0, categorie.length - 1)
        : categorie
    }.json`
  )
